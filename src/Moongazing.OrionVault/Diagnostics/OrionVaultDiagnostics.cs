...
/// <summary>
/// OpenTelemetry instrumentation for field encryption. Built on the Orion family's
/// <see cref="OrionInstrumentation"/> spine, so it shares the family's naming conventions: a
/// <see cref="Meter"/> and <see cref="ActivitySource"/> named <c>Moongazing.OrionVault</c> (subscribe
/// by that name) exposing the encryption/decryption/key counters, histograms, and gauges under the
/// <c>orion.vault.*</c> names. Registered as a singleton; dispose it to release the meter and the
/// activity source.
/// </summary>
public sealed class OrionVaultDiagnostics : OrionInstrumentation
{
    ...
    public OrionVaultDiagnostics()
        : base(OrionTelemetry.ScopeName("OrionVault"), MeterVersion.Value)
    {
        Encryptions = Meter.CreateCounter<long>("orion.vault.encryptions", "{operations}",
            "Number of encryption operations performed.");
        Decryptions = Meter.CreateCounter<long>("orion.vault.decryptions", "{operations}",
            "Number of decryption operations performed.");
        DecryptionFailures = Meter.CreateCounter<long>("orion.vault.decryption.failures", "{operations}",
        ...
        AuthTagFailures = Meter.CreateCounter<long>("orion.vault.decryption.auth_tag_failures", "{operations}",
        ...
        EncryptionFailures = Meter.CreateCounter<long>("orion.vault.encryption.failures", "{operations}",
            "Number of failed encryptions, tagged by reason.");
        KeyLookups = Meter.CreateCounter<long>("orion.vault.key_lookups", "{operations}",
        ...
        DecryptionDuration = Meter.CreateHistogram<double>("orion.vault.decryption.duration_ms", "ms",
            "Duration of decrypt operations (success and failure).");
        ReEncryptionRowsProcessed = Meter.CreateCounter<long>("orion.vault.reencryption.rows_processed", "{rows}",
        ...
        RotationRowsSkipped = Meter.CreateCounter<long>("orion.vault.rotation.rows_skipped", "{rows}",
        ...
        EncryptionPayloadSize = Meter.CreateHistogram<int>(
            "orion.vault.encryption.payload_size_bytes", "By",
        ...
        DecryptionPayloadSize = Meter.CreateHistogram<int>(
            "orion.vault.decryption.payload_size_bytes", "By",
        ...
        CiphertextOverheadRatio = Meter.CreateHistogram<double>(
            "orion.vault.encryption.ciphertext_overhead_ratio", "1",
        ...
        _ = Meter.CreateObservableGauge<long>("orion.vault.keys.registered_count",
        ...
        LegacyKeyUsedOnDecrypt = Meter.CreateCounter<long>(
            "orion.vault.decryption.legacy_key_used", "{operations}",
        ...
        KeyResolutionDuration = Meter.CreateHistogram<double>(
            "orion.vault.key_resolution.duration_ms", "ms",
        ...
        _ = Meter.CreateObservableGauge<long>("orion.vault.rotation.last_cycle.scanned", () => Interlocked.Read(ref lastCycleScanned),
            "{rows}", "Rows the last rotation cycle scanned.");
        _ = Meter.CreateObservableGauge<long>("orion.vault.rotation.last_cycle.rotated", () => Interlocked.Read(ref lastCycleRotated),
            "{rows}", "Rows the last rotation cycle rotated.");
        _ = Meter.CreateObservableGauge<long>("orion.vault.rotation.last_cycle.skipped", () => Interlocked.Read(ref lastCycleSkipped),
        ...
        _ = Meter.CreateObservableGauge<long>("orion.vault.active_key_id",
